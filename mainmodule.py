...
mixer.init()
alarm_sound = mixer.Sound(r"C:\Users\bjaya\Desktop\projects\Project_internship\mixkit-vintage-warning-alarm-990.wav")
# this xml files are used to detect face , left eye , and right eye of a person.
face_detection = cv2.CascadeClassifier(r"C:\Users\bjaya\Desktop\projects\Project_internship\haarcascade_frontalface_alt.xml")
left_eye_detection = cv2.CascadeClassifier(r"C:\Users\bjaya\Desktop\projects\Project_internship\haarcascade_lefteye_2splits.xml")
right_eye_detection = cv2.CascadeClassifier(r"C:\Users\bjaya\Desktop\projects\Project_internship\haarcascade_righteye_2splits.xml")


labels =['Close','Open']

# load the model, that we have created
model = load_model(r'C:\Users\bjaya\Desktop\custmodel.h5')
...
while(True):
    ...
    if(right_eye_pred[0] == 0 and left_eye_pred == 0):
        ...
        for (x,y,w,h) in right_eye:
            cv2.rectangle(frame, (x,y) , (x+w,y+h) , (0,200,10) , 1 )
        for (x,y,w,h) in left_eye:
            cv2.rectangle(frame, (x,y) , (x+w,y+h) , (0,200,10) , 1 )
        time += 1
    ...
    # if(right_eye_pred[0]==1 or left_eye_pred[0]==1):
        ...
        for (x,y,w,h) in faces:
            cv2.rectangle(frame, (x,y) , (x+w,y+h) , (255,0,25) , 3 )
        for (x,y,w,h) in right_eye:
            cv2.rectangle(frame, (x,y) , (x+w,y+h) , (20,1,20) , 0 )
        for (x,y,w,h) in left_eye:
            cv2.rectangle(frame, (x,y) , (x+w,y+h) , (20,1,20) , 0 )
        time -= 1
